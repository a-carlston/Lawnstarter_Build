...
splits <- time_series_split(data_prepared_current_tbl, assess = "11 weeks", cumulative = TRUE)
...
# Assuming you have a data frame named 'Models_Forecast' with columns Date, .key, Forecast, and holiday_name_event

# Modify the data frame and assign it to a variable
modified_df <- Models_Forecast %>%
  select(Date = .index, .key, Forecast = .value, holiday_name_event) %>%
  mutate(holiday_name_event = ifelse(holiday_name_event == 0, NA, holiday_name_event))
